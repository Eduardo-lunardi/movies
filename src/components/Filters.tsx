...
export default function Filters(props: IProps): React.ReactElement {
    ...
    function check(type: order): void {
        const value: IFilter = { [type]: filter ? filter[type] : undefined, }
        if (value[type] === 'cres')
            value[type] = 'desc'
    ...
    return (
        <ol className="filters">
            ...
            <li onClick={(): void => check('year')}>
                <label className={`${filter && filter.year !== undefined && 'check'}`}>
                    Year {icons('year')}
                </label>
            </li>
            <li onClick={(): void => check('country')}>
                <label className={`${filter && filter.country !== undefined && 'check'}`}>
